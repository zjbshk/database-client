...
import static org.jasonsui.commons.FxmlConstants.PG_CONFIG;
...
public class PGConfigView extends AbstractFxmlView implements Initializable {
    ...
    private void initVal() {
        ...
        portTextF.setText("5432");
        urlTextF.setText("jdbc:postgresql://localhost:5432");
        userTextF.setText("postgres");
        dbConfigModal
    ...
    public void setUrlTextF(Change<String> change) {
        ...
        if (split.length > 1) {
            urlTextF.setText(String.format("jdbc:postgresql://%s:%s/%s?%s", hostTextF.getText(), portTextF.getText(),
                    databaseTextF.getText(), split[1]));
        } else {
            urlTextF.setText(String.format("jdbc:postgresql://%s:%s/%s", hostTextF.getText(), portTextF.getText(),
